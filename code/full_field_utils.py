...
def write_out_stitched_full_field_image(
    path_to_avg_tiff: Path,
    path_to_full_field_tiff: Path,
    output_path: Path,
):
    """
    ...
    be written to the logging, but the overall process will
    ...
    try:
        ...
        full_field_img = stitch_full_field_tiff(path_to_full_field_tiff)

        avg_splitter = AvgImageTiffSplitter(path_to_avg_tiff)
        ...
        with h5.File(output_path, "w") as out_file:
            out_file.create_dataset("stitched_full_field", data=full_field_img)

            out_file.create_dataset("stitched_full_field_with_rois", data=with_rois)

            out_file.create_dataset(
                "surface_roi_metadata", data=json.dumps(avg_splitter.raw_metadata).encode("utf-8")
            )

            ff_metadata = json.dumps(full_field_metadata.raw_metadata)
            out_file.create_dataset("full_field_metadata", data=ff_metadata.encode("utf-8"))
    except Exception:
...
def get_full_field_path(runner_args: dict) -> Optional[Path]:
    """
    ...
    if platform_key not in runner_args or runner_args[platform_key] is None:
        logging.warning(
            "platform_json_path not specified; " "skipping stitched full field image generation"
        )
        return None
    ...
    paths_to_check = []
    paths_to_check.append(Path(runner_args["storage_directory"]) / ff_name)

    upload_dir_key = "data_upload_dir"
    if upload_dir_key in runner_args:
        if runner_args[upload_dir_key] is not None:
            paths_to_check.append(Path(runner_args[upload_dir_key]) / ff_name)
    ...
    logging.info("Getting fullfield_2p image from " f"{full_field_path.resolve().absolute()}")
    return full_field_path


def stitch_tiff_with_rois(
    full_field_path: Union[pathlib.Path, str], avg_surface_path: Union[pathlib.Path, str]
) -> np.ndarray:
    """
...
def stitch_full_field_tiff(tiff_path: pathlib.Path) -> np.ndarray:
    """
    ...
    img = _stitch_full_field_tiff(tiff_metadata=tiff_metadata, avg_img=img)
...
def _insert_rois_into_surface_img(
    ...
    (ff_resolution, ff_size) = _validate_all_roi_same_size(full_field_metadata)

    (origin_col, origin_row) = _get_origin(full_field_metadata)

    output_img = np.copy(full_field_img)

    physical_to_pixels = (ff_resolution[0] / ff_size[0], ff_resolution[1] / ff_size[1])
    ...
    for i_roi in range(avg_image_splitter.n_rois):
        ...
        roi_conversion = (roi_resolution[0] / roi_size[0], roi_resolution[1] / roi_size[1])
        ...
        # full field image.
        rescaling_factor = (
            physical_to_pixels[0] / roi_conversion[0],
        ...
        avg_img = skimage_resize(avg_img, output_shape=(new_rows, new_cols))

        avg_img = normalize_array(array=avg_img, dtype=np.uint16)
        ...
        output_img[row0:row1, col0:col1] = avg_img

    return output_img


def _average_full_field_tiff(tiff_path: pathlib.Path) -> np.ndarray:
    """
...
def _get_stitched_tiff_shapes(tiff_metadata: ScanImageMetadata, avg_img: np.ndarray) -> Dict:
    """
...
def _get_origin(metadata: ScanImageMetadata) -> Tuple[float, float]:
    """
...
def _stitch_full_field_tiff(tiff_metadata: ScanImageMetadata, avg_img: np.ndarray) -> np.ndarray:
    """
    ...
    final_shapes = _get_stitched_tiff_shapes(tiff_metadata=tiff_metadata, avg_img=avg_img)
    ...
    (resolution, physical_size) = _validate_all_roi_same_size(tiff_metadata)

    physical_to_pixels = (resolution[0] / physical_size[0], resolution[1] / physical_size[1])

    (origin_col, origin_row) = _get_origin(tiff_metadata)
    ...
    for i_roi in range(tiff_metadata.n_rois):
        ...
        sub_img = avg_img[
            i_roi * (resolution[1] + pixel_gap) : i_roi * pixel_gap + (i_roi + 1) * resolution[1], :
        ]

        stitched_img[
            pix_row0 : pix_row0 + resolution[1], pix_col0 : pix_col0 + resolution[0]
        ] = sub_img
